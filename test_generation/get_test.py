    ...
    def parse_conditional_line(self, line):
        ...
        The method:
          ...
          2. Splits the remaining text at the first occurrence of whitespace that precedes an article
             (THEN, THE, ALL, SOME,or ANY) using a positive lookahead with a non-capturing group.
        """
        ...
        #DO NOT REMOVE BEFORE BECAUSE BEFORE MEANS THE CONDITION HAS NOT OCCURED YET
        line_no_cond = re.sub(r'^(IF|WHILE|DURING|AFTER)\s+', '', line, flags=re.IGNORECASE)
        # Split on the first whitespace before an article (THE, A, or AN).
        #parts = re.split(r'\s+(?=(?:THEN|THE|A|AN)\b)', line_no_cond, maxsplit=1)
        parts = re.split(r'\s+(?=(?:THEN|THE|ALL|ANY|SOME)\b)', line_no_cond, maxsplit=1)
        if len(parts) >= 2:
            pre = parts[0].strip()
            beh = parts[1].strip()
            return pre, beh
